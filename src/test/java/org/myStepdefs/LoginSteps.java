...
public class LoginSteps {
    ...
    public void userFillTheInvalidPasswordDetailsOnPasswordField(String invalidPassword) {
        String password = PropertiesUtils.getProperty(PropertyFileEnum.CREAD, invalidPassword);
        MM_LoginScreen login = new MM_LoginScreen();
        login.sendKeys_onPassword(password);
        logger.info("Invalid password entered successfully : "+password);
    }
    ...
    public void theErrorValidationMessagesOnEmailDisplaySuccessfully() {
        MM_LoginScreen login = new MM_LoginScreen();
        Assert.assertTrue(login.isErrorMessageDisplayed());
        Assert.assertEquals(login.getErrorTextMessage(),"Invalid email/password");
